    ...
    <div class="col-sm-10">
        <select name="room_id" class="form-control" id="room_id" required>
            @foreach($rooms as $id => $display)
                ...
                {{-- right above we did some of the roomid reference to be set in case that we already had selected when we first created this --}}
            @endforeach
        </select>
...
<div class="form-group row">
    ...
    <input name="start" type="date" class="form-control" required placeholder="yyyy-mm-dd" value="{{ $booking->start ?? ''}}"/>
        {{-- the value part above means that in case that the first variable in the operator isnt null it will return its value in case it is null it will return the second part of the operator (a empty string) --}}
...
<!-- this prevents cross side requests from be done -->
@csrf